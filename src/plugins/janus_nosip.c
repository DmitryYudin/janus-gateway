...
static void janus_nosip_hangup_media_internal(janus_plugin_session *handle) {
	...
	if(!session->media.ready) {
		janus_nosip_relay_thread_close_internal(session);
	}
...
static int bind_socket_pair(int rtp_fd, int rtcp_fd, int rtp_port, int rtcp_port) {
	if(bind_socket(rtp_fd, rtp_port)) {
		return -1;
	}
	if(bind_socket(rtcp_fd, rtcp_port)) {
		return -1;
	}
	return 0;
}
