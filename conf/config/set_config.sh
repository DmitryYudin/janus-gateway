...
set_environment()
{
	...
	##
	# RabbitMQ (exhcange must be started, no reconnect supported)
	rabbitmq_enable=${JANUS_RABBITMQ_HOST:+'yes'}
	rabbitmq_events=${JANUS_RABBITMQ_EVENTS:-'all'}
	rabbitmq_host=${JANUS_RABBITMQ_HOST:-'localhost'}
	rabbitmq_port=${JANUS_RABBITMQ_PORT:-'5672'}

	##
	# MQTT events
	mqttevh_enable='no'
	mqttevh_url='tcp://localhost:5006'

	##
	# HTTP events
	sampleevh_enable=${JANUS_LS_HTTP_HOST:+'yes'}
	JANUS_LS_HTTP_HOST=${JANUS_LS_HTTP_HOST-'elk'}
	JANUS_LS_HTTP_PORT=${JANUS_LS_HTTP_PORT-5002}
	sampleevh_backend="http://${JANUS_LS_HTTP_HOST}:${JANUS_LS_HTTP_PORT}"

	##
	# Prometheus events
	prometheus_enable=${JANUS_PROMETHEUS_HOST:+'yes'}
	prometheus_host=$JANUS_PROMETHEUS_HOST
	prometheus_port=$JANUS_PROMETHEUS_PORT
	...
	cfg_set 'cert_pem' $datadir'/janus/certs/mycert.pem'
	cfg_set 'cert_key' $datadir'/janus/certs/mycert.key'
	...
	# just to suppress error message on cfg read
	cfg_file 'janus.transport.mqtt'

	cfg_file 'janus.plugin.nosip'
	cfg_set 'local_ip' $nosip_local_ip
	cfg_set 'rtp_port_range' $nosip_port_range

	cfg_file 'janus.eventhandler.rabbitmqevh'
	cfg_set 'enabled' $rabbitmq_enable
	cfg_set 'events' $rabbitmq_events
	cfg_set 'host' $rabbitmq_host
	cfg_set 'port' $rabbitmq_port

	cfg_file 'janus.eventhandler.mqttevh'
	cfg_set 'enabled' $mqttevh_enable
	cfg_set 'url' $mqttevh_url

	cfg_file 'janus.eventhandler.sampleevh'
	cfg_set 'enabled' $sampleevh_enable
	cfg_set 'backend' $sampleevh_backend

	cfg_file 'janus.eventhandler.prometheusevh'
	cfg_set 'enabled' $prometheus_enable
	cfg_set 'host' $prometheus_host
	cfg_set 'port' $prometheus_port
